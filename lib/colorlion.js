/* colorlion.js ver.1.18 by Colorlion (Do not remove this line)*/
(function () {
  function colorlion(color, elmnt) {
    if (!(this instanceof colorlion)) {
      return new colorlion(color, elmnt);
    }
  ...
  window.colorlion = colorlion;
})();

function lionSetColorsByAttribute() {
  var z, i, att;
  z = document.getElementsByTagName("*");
  for (i = 0; i < z.length; i++) {
    att = z[i].getAttribute("data-lion-color");
    if (att) {
      z[i].style.backgroundColor = colorlion(att).toRgbString();
    }
  }
}
